       ...
       [
        (try_for_agents,":agent_no"),
		  ...
		  (try_begin),
		    ...
		    (copy_position,pos20,pos15),
		    (copy_position,pos21,pos15),
		    (position_move_x, pos18, 400),#1.5m
		...
		(try_begin),
		   ...
		   (try_begin),
			   ...
			   (try_for_range,":weapon_slot",ek_item_0,ek_item_3+1),
					...
					(init_position, pos1),
					(position_copy_origin, pos1, pos3),
					(call_script,"script_adimi_tool_generate_random_spawn_position",60),#-60 up to 60
					(spawn_item, ":weapon", 0,999999),#999999 not 0  if you set 0, it will also not vanish after map reset or round ending
				(try_end),
				...
				(try_end),
				(try_begin),
					(assign,reg1,":current_count"),
					(try_begin),
						(neq,":killer_agent_no",":dead_agent_no"),
						(agent_is_active,":killer_agent_no"),
						(neg|agent_is_non_player,":killer_agent_no"),
						(agent_get_player_id,":killer_agent_playerid",":killer_agent_no"),
						(player_is_active,":killer_agent_playerid"),
						(str_store_player_username,s2,":killer_agent_playerid"),
						(str_store_player_username,s3,":dead_agent_playerid"),
						(str_store_string,s62,"str_adimi_tool_hungergames_player_kill"),
						(call_script,"script_adimi_tool_server_message_alternativ"),
					(else_try),
						(str_store_player_username,s3,":dead_agent_playerid"),
						(str_store_string,s62,"str_adimi_tool_hungergames_player_suicid"),
						(call_script,"script_adimi_tool_server_message_alternativ"),
					(try_end),
					#Added specification for suicide or player a killed player b
					#(str_store_string,s62,"str_adimi_tool_hungergames_players_left"),
					#(call_script,"script_adimi_tool_server_message_alternativ"),
				(try_end),
				(this_or_next|eq,":current_count",0), #04:04 30.07.2014 added 0 also.. may both players die.. or something similiar
				(eq,":current_count",1),
				(assign,"$adimi_tool_hungergames_reset_map",1),
			(else_try),
			    #Remove always the weapons...
				(try_for_range,":weapon_slot",ek_item_0,ek_item_3+1),
					(agent_get_item_slot, ":weapon",":dead_agent_no",":weapon_slot"),
					(is_between, ":weapon",all_items_begin,all_items_end),
					(agent_unequip_item, ":dead_agent_no", ":weapon"),
       ...
       [
		 ...
		 (try_begin),
		   ...
		   (try_for_players,":playerid"),
				...
				(player_set_slot,":playerid",adimi_tool_player_is_in_hungergame,1),
				(player_get_agent_id,":agent",":playerid"),
				(player_get_slot,":prop_instance_id",":playerid",adimi_tool_hungergames_spawn_scene_prob_id),
				(prop_instance_get_position, pos0, ":prop_instance_id"), 
				(player_set_troop_id, ":playerid","trp_hired_assassin"),
				(try_begin),
					(agent_is_active,":agent"),
					(agent_is_alive,":agent"),
					(neg|agent_is_non_player,":agent"),
					...
					#unequip all weapons
					(try_for_range,":weapon_slot",ek_item_0,ek_item_3+1),
						(agent_get_item_slot, ":weapon",":agent",":weapon_slot"),
						(is_between, ":weapon",all_items_begin,all_items_end),
						(agent_unequip_item, ":agent", ":weapon"),
					(try_end),
					#reset 4 playerid only
					(player_set_slot,":playerid",adimi_tool_hungergames_spawn_scene_prob_id,-1),
				(else_try),# If the player is spec... spawn a new agent.. set a troop, team etc. and teleport him/her to the choosen spawn position
					(player_set_team_no,":playerid",0),
					(assign, ":armor", "itm_heraldic_mail_with_tunic"),#Armor stats changed! Body 10 legs 5
					(player_add_spawn_item, ":playerid", ek_foot, "itm_mail_boots_for_tableau"),
					(player_add_spawn_item,":playerid", ek_body, ":armor"),
					(player_add_spawn_item,":playerid", ek_head, "itm_items_end"),
					(player_add_spawn_item,":playerid", ek_gloves, "itm_leather_gloves"),
					(player_add_spawn_item, ":playerid", ek_item_0, "itm_items_end"),
					(player_add_spawn_item,":playerid", ek_item_1, "itm_items_end"),
					(player_add_spawn_item,":playerid", ek_item_2, "itm_items_end"),
					(player_add_spawn_item,":playerid", ek_item_3, "itm_items_end"),
					(player_add_spawn_item, ":playerid", ek_horse, "itm_items_end"),
					(player_spawn_new_agent,":playerid",1),
					(player_set_slot,":playerid",adimi_tool_rdy_for_hungergames_respawning,1),	
		   ...
		   # (try_for_range,":playerid",0,":max"),
		    (try_for_players,":playerid"),
				...
				(try_for_players, ":second_playerid"), 
					...
					(player_slot_eq,":second_playerid",adimi_tool_player_is_in_hungergame,1),
					(player_get_agent_id,":second_agentid",":second_playerid"),
					(agent_is_active,":second_agentid"),
				    (agent_is_alive,":second_agentid"),
					(agent_add_relation_with_agent,":second_agentid", ":agent",-1),##ENEMY
...
multiplayer_server_check_polls = (
  ...
  [
    ...
    (this_or_next|is_between, "$adimi_tool_poll_override", 1,3),#Adimi Poll Override
...
##  ),
    ...
    [
      ...
      (ti_on_agent_spawn, 0, 0, [(neq,"$adimi_tool_hungergames",1)],
       [
         (store_trigger_param_1, ":agent_no"),
         (call_script, "script_multiplayer_server_on_agent_spawn_common", ":agent_no"),
         ]),

      (ti_server_player_joined, 0, 0, [(neq,"$adimi_tool_hungergames",1)],
       [
         (store_trigger_param_1, ":player_no"),
         (call_script, "script_multiplayer_server_player_joined_common", ":player_no"),
         ]),

      (ti_before_mission_start, 0, 0, [(neq,"$adimi_tool_hungergames",1)],
       [
         (assign, "$g_multiplayer_game_type", multiplayer_game_type_deathmatch),
         (call_script, "script_multiplayer_server_before_mission_start_common"),
         ...
         (call_script, "script_adimi_tool_set_hungergamesmode_automatic"), #Adimi Tool Checks 4 Hungergames, gamemode 01:59 18.07.2014
         (call_script, "script_multiplayer_init_mission_variables"),
         (call_script, "script_multiplayer_remove_destroy_mod_targets"),
         (call_script, "script_multiplayer_remove_headquarters_flags"), # close this line and open map in deathmatch mod and use all ladders firstly 
         ]),                                                            # to be able to edit maps without damaging any headquarters flags ext. 

      (ti_after_mission_start, 0, 0, [(neq,"$adimi_tool_hungergames",1)], 
       [
         (set_spawn_effector_scene_prop_kind, 0, -1), #during this mission, agents of "team 0" will try to spawn around scene props with kind equal to -1(no effector for this mod)
         (set_spawn_effector_scene_prop_kind, 1, -1), #during this mission, agents of "team 1" will try to spawn around scene props with kind equal to -1(no effector for this mod)
      ...
      (ti_on_multiplayer_mission_end, 0, 0, [(neq,"$adimi_tool_hungergames",1)],
       [
         #ELITE_WARRIOR achievement
         (try_begin),
           (multiplayer_get_my_player, ":my_player_no"),
      ...
      (ti_on_agent_killed_or_wounded, 0, 0, [(neq,"$adimi_tool_hungergames",1)],
       [
         (store_trigger_param_1, ":dead_agent_no"), 
         (store_trigger_param_2, ":killer_agent_no"),
         (call_script, "script_multiplayer_server_on_agent_killed_or_wounded_common", ":dead_agent_no", ":killer_agent_no"),
      ...
      (1, 0, 0, [(neq,"$adimi_tool_hungergames",1)],
       [
         (multiplayer_is_server),
         (get_max_players, ":num_players"),
         (try_for_range, ":player_no", 0, ":num_players"),
      ...
      (1, 0, 0, [(neq,"$adimi_tool_hungergames",1)], #do this in every new frame, but not at the same time
       [
         (multiplayer_is_server),
         (store_mission_timer_a, ":mission_timer"),
         (ge, ":mission_timer", 2),
      ...
      (0, 0, 0, [(neq,"$adimi_tool_hungergames",1)],
       [
         (multiplayer_is_server),
         (eq, "$g_multiplayer_ready_for_spawning_agent", 1),
         (store_add, ":total_req", "$g_multiplayer_num_bots_required_team_1", "$g_multiplayer_num_bots_required_team_2"),
    ...
    [
      ...
      (1, 0, 3, [(multiplayer_is_server),
       ...
       [
		 ...
		 ##AdimiTools Switch Teams After Round
		 (call_script,"script_adimi_tool_siege_team_switch_after_round"),
		 ##
    ...
    [
      ...
      (1, 0, 0, [(multiplayer_is_server),
				...
				(else_try),#Usual flag spawn
				  ...
				  (assign,"$adimi_tool_mof",2),
				(try_end),
				(this_or_next|eq,"$adimi_tool_mof",1),
				(eq,"$adimi_tool_mof",2),
		],
       [
	     #AdimiTools Start WNL 2min MoF 
      ...
      (1, 0, 0, [(multiplayer_is_server),
				 ...
				 #AdimiTools Start WNL 2min MoF 
				(try_begin), 
				 (eq,"$adimi_tool_mof",2),#call it just if 2min mof is disable
      ...
      (1, 0, 3, [(multiplayer_is_server),
       ...
       [
         ...
         #clear scene and end round
         (multiplayer_clear_scene),
         (call_script,"script_adimi_tool_siege_team_switch_after_round"),#AdimiTool Team switch
         (call_script, "script_multiplayer_initialize_belfry_wheel_rotations"),
